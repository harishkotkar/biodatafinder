...
public class ProfileController {
    ...
    // GET a profile by ID
    @GetMapping("/{id}")
    public ResponseEntity<Profile> getPersonById(@PathVariable(value = "id") String personId)
            throws ResourceNotFoundException {
        Profile profile = profileRepository.findById(personId)
                .orElseThrow(() -> new ResourceNotFoundException("Profile not found for this id :: " + personId));
        return ResponseEntity.ok().body(profile);
    }

    // CREATE a new profile
    @PostMapping("")
    public Profile createPerson(@RequestBody Profile profile) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Profile> updatePerson(@PathVariable(value = "id") String personId,
                                                @RequestBody Profile personDetails) throws ResourceNotFoundException {
        Profile profile = profileRepository.findById(personId)
                .orElseThrow(() -> new ResourceNotFoundException("Profile not found for this id :: " + personId));
